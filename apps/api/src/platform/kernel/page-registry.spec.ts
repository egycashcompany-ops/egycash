...
describe('the assembled page registry', () => {
  ...
  it('declares 69 pages over 257 permissions', () => {
    expect(pages).toHaveLength(69);
    expect(permissions).toHaveLength(257);
  });
   ...
   */
  it('assigns 230 permissions to a page and leaves 27 deliberately unassigned', () => {
    const assigned = permissions.filter((p) => p.pageId !== null);
    expect(assigned).toHaveLength(230);
  ...
  it('splits the pages across the four modules as declared', () => {
    ...
    expect(Object.fromEntries(byModule)).toEqual({ platform: 15, hr: 35, fleet: 10, it: 9 });
  });
