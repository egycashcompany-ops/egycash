...
export const listJobTitles = async (req: Request, res: Response): Promise<void> => {
  ...
  ok(res, await jobTitleService.toDtos(page.items), page.meta);
};
...
export const createJobTitle = async (req: Request, res: Response): Promise<void> => {
  ...
  created(res, await jobTitleService.toDtoNamed(doc), `/api/v1/platform/job-titles/${String(doc._id)}`);
};
...
export const updateJobTitle = async (req: Request, res: Response): Promise<void> => {
  ...
  ok(res, await jobTitleService.toDtoNamed(await jobTitleService.update(params.id, body, ctx.userId)));
};
