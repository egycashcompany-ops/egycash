// Thin HTTP mapping only (ADR-003). Every handler passes the caller's context: the per-unit
...
export const setUserDelegation = async (req: Request, res: Response): Promise<void> => {
  const { body, params } = validated<SetDelegation, never, { userId: string }>(req);
  ok(res, await delegationService.set(authContext(req), params.userId, body));
};
