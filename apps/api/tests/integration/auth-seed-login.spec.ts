...
describe('seed → password login (regression)', () => {
  ...
  it('re-running the seed is idempotent — no duplicate categories/applications/grants', async () => {
    ...
    expect(groups.reduce((n, g) => n + g.applications.length, 0)).toBe(50);
  });
