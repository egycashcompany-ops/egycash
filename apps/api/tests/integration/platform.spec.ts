...
describe('login → permission → scoped data → audit trail', () => {
  ...
  it('assigns and removes Department ↔ Application links (PR #62)', async () => {
    const dept = await request(app)
      .post('/api/v1/platform/departments')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ code: 'DEP-DA', name: { ar: 'إدارة التطبيقات', en: 'Apps Dept' }, branchId: branchAId });
    const deptId = (dept.body as { data: { id: string } }).data.id;

    const cat = await request(app)
      .post('/api/v1/platform/application-categories')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ name: { ar: 'فئة', en: 'Cat' } });
    ...
    // Listing the department's applications shows the assigned one.
    const listed = await request(app)
      .get(`/api/v1/platform/departments/${deptId}/applications`)
      .set('Authorization', `Bearer ${adminToken}`);
    expect(listed.status).toBe(200);
    ...
    // Remove → 204; the department and application still exist afterwards.
    const removed = await request(app)
      .delete(`/api/v1/platform/departments/${deptId}/applications/${appId}`)
      .set('Authorization', `Bearer ${adminToken}`);
    expect(removed.status).toBe(204);
    ...
    expect((await request(app).get(`/api/v1/platform/departments/${deptId}`).set('Authorization', `Bearer ${adminToken}`)).status).toBe(200);
  ...
  it("resolves the caller's effective applications: derived from permissions, active-only, grouped, ordered (PR #64)", async () => {
    ...
    const willDeactivate = await mkApp('ME-OFF', hrCat, 0, 'costCenter.view');
