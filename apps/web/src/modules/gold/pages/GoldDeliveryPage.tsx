// عمليات الخروج — the delivery-order list.
import { useMemo, useState } from 'react';
import { useSearchParams } from 'react-router-dom';
import { type GoldDeliveryReceiptDto } from '@ecms/contracts';
...
import { useGoldDelivery, useGoldDeliveryList, useRevertGoldDelivery } from '../api/gold-queries';
import { BranchTag } from '../components/BranchTag';
import { DeliveryEditorDialog } from '../components/DeliveryEditorDialog';
import {
  docStatusLabel,
  docStatusOptions,
  docStatusTone,
...
const EditorLoader = ({
  ...
  const { data, isFetching } = useGoldDelivery(target);
  return (
...
export const GoldDeliveryPage = (): JSX.Element => {
  ...
  const { data, isLoading, isError, error, refetch } = useGoldDeliveryList(params);
  const owners = useGoldCompanyOptions();
  const revert = useRevertGoldDelivery();
  const [target, setTarget] = useState<string | null | undefined>(undefined);

  const onRevert = async (row: GoldDeliveryReceiptDto): Promise<void> => {
    if (!window.confirm(t('gold.delivery.revertPrompt'))) return;
    try {
  ...
  const columns: Column<GoldDeliveryReceiptDto>[] = [
    { key: 'receiptDate', header: t('gold.common.date'), render: (r) => fmtDate(r.receiptDate) },
    {
      key: 'receiptNumber',
      header: t('gold.delivery.number'),
      render: (r) => (
    ...
    { key: 'company', header: t('gold.delivery.owner'), render: (r) => r.companyName ?? '—' },
    ...
    },
    {
      ...
      render: (r) => (
        <div className="flex justify-end gap-1">
          ...
          {r.status === 'confirmed' && (
            <Can permission="goldDelivery.revert">
              <Button
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        actions={
          <Can permission="goldDelivery.create">
            <Button
              size="sm"
      ...
      <div className="space-y-4">
        <FilterBar
          hasActiveFilters={
            search !== '' || companies.length > 0 || statuses.length > 0 || metals.length > 0
          }
          onClear={() => {
            patch({ q: null, company: null, status: null, metal: null });
          }}
        >
          <SearchInput
            value={search}
            ...
            placeholder={t('gold.delivery.searchPlaceholder')}
            className="w-64"
          />
          <MultiSelect
