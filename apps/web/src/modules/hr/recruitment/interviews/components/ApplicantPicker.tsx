// Applicant search-picker (autocomplete) for the interview queue filter + schedule dialog.
// Debounced search against the Applicants list API; selecting one returns the full DTO. RTL-safe.
  ...
  return (
    <div className="relative w-full sm:w-72" ref={ref}>
      <SearchInput
        ...
        placeholder={placeholder ?? t('interviews.filters.applicantSearch')}
      />
      {open && term.trim().length >= 2 && (
        <div className="absolute z-20 mt-1 w-full overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg dark:border-slate-700 dark:bg-slate-800">
          {isFetching ? (
            ...
            <p className="px-3 py-3 text-sm text-slate-400">{t('interviews.filters.noApplicants')}</p>
          ) : (
