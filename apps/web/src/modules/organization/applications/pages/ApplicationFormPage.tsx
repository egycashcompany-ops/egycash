// Application create & edit. Name is required bilingual; icon, client route and category are required
// single strings; sort order is a non-negative integer (defaults to 0). Edits are version-checked; a
// stale save surfaces as a toast.
...
const ApplicationFormBody = ({ existing }: { existing: ApplicationDto | null }): JSX.Element => {
  ...
  const submit = async (): Promise<void> => {
    ...
    if (icon.trim() === '' || route.trim() === '' || category.trim() === '') {
