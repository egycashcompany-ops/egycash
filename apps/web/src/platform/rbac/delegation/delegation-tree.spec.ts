...
// ── The reader's own narrowing ──────────────────────────────────────────────
//
// An owner delegating for a colleague is shown every branch, every department in it, and the whole
// registry inside each. That is a true answer to «what may I grant here» and a useless one to
// «where is the leave screen», so the panel hands down a predicate. What matters is WHERE it runs:
// on top of the entitlement filter, never instead of it — narrowing may only ever remove rows the
// caller was already entitled to see, and it may never reach the records.
describe('a screen filter narrows what is drawn', () => {
  const tree = (filter: Parameters<typeof buildTree>[4]): BranchNode[] =>
    buildTree(CATALOG, () => NOTHING, () => NOTHING, new Set(), filter);

  it('keeps only the screens that match', () => {
    const narrowed = tree((screen) => screen.page?.id === 'fleet.vehicles');
    const october = narrowed.find((b) => b.id === 'B');
    expect(october?.departments[0]?.modules.flatMap((m) => m.screens).map((s) => s.id)).toEqual([
      'fleet.vehicles',
    ]);
  });

  it('drops a department whose every screen was filtered away, and then its branch', () => {
    // The `empty` rule already in place does this — the filter needs no logic of its own for it,
    // which is the point: a heading over nothing is the same defect whether the emptiness came
    // from the caller's ceiling or from his search box.
    const narrowed = tree((screen) => screen.page?.id === 'hr.employees');
    expect(narrowed.map((b) => b.id)).toEqual(['A']);
  });

  it('can filter the whole tree away, and says so by being empty', () => {
    expect(tree(() => false)).toEqual([]);
  });

  it('never widens: a screen outside the ceiling stays out however the filter is written', () => {
    // `setting.manage` is October's alone. A predicate that accepts everything must not put it in
    // Mohandessin, where the caller may not grant it and the account does not hold it.
    const everything = tree(() => true);
    const mohandessin = everything.find((b) => b.id === 'A');
    const keys = mohandessin?.departments.flatMap((d) => [...d.ceiling]) ?? [];
    expect(keys).not.toContain('setting.manage');
  });

  it('leaves the records alone — the ceiling and the saved set are what they were', () => {
    // The filter decides what is painted. A draft written on a unit the reader then filters out of
    // sight is still his work, and the panel reads the save set off an UNFILTERED tree for exactly
    // this reason; here the point is that narrowing did not quietly empty the unit's own state.
    const held = new Set(['vehicle.view']);
    const narrowed = buildTree(CATALOG, () => held, () => held, new Set(), () => false);
    const wide = buildTree(CATALOG, () => held, () => held, new Set(), () => true);
    expect(narrowed).toEqual([]);
    expect(wide.find((b) => b.id === 'B')?.departments[0]?.selected.has('vehicle.view')).toBe(true);
  });
});
