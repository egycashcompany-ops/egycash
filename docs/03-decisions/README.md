...
| [ADR-032](ADR-032-outbound-http-one-door.md) | Outbound HTTP leaves through one guarded door — host allowlist, private-address refusal, redirects judged per hop, and a guard spec that forbids a bare `fetch` anywhere else | Accepted |
| [ADR-033](ADR-033-email-is-opt-in-per-send.md) | Email is asked for at the moment of sending, never assumed — the platform never emails on its own initiative | Accepted |
