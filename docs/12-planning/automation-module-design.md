...
| **A-5** ✅ | Trigger bridge: one subscription per cataloged event → filter eval → idempotent execution rows → provider dispatch; introduces `automation_executions` | `modules/automation/triggers/**`, `executions/**` | filter eval (12 unit); dispatch, filter miss, idempotency, depth guard, no-provider skip, snapshot redaction (integration) | `automation_executions` |
