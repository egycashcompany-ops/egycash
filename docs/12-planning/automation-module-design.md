...
| **A-3** ✅ | Registry: workflows + variables (model, repo, service, routes), module manifest behind the flag, catalogue-backed trigger validation, suspend-on-owner-deactivation, `GET /events` | `modules/automation/**`, `modules/index.ts` | trigger validation (19 unit); CRUD, ownership, enable gate, transfer, suspension, variables, catalogue ETag (integration) | `automation_workflows`, `automation_variables` |
| **A-4** ✅ | Credentials: write-only sealed store, pure redaction, unattended key rotation | `modules/automation/credentials/**` | redaction (17 unit); no read path incl. guessed routes, no plaintext in the collection, ciphertext-swap refused, rotation round-trip, permission split (integration) | `automation_credentials` |
| **A-5** ✅ | Trigger bridge: one subscription per cataloged event → **enqueue** → worker filter eval → idempotent execution rows → provider dispatch; a reusable `N8nClient` + `N8nAutomationProvider` (trigger path only) behind A-0; introduces `automation_executions` and the `ModuleManifest.jobHandlers` seam | `modules/automation/triggers/**`, `executions/**`, `platform/automation/providers/n8n/**` | filter eval (12 unit), n8n client transport (8 unit), n8n provider (6 unit); dispatch, filter miss, idempotency, depth guard, no-provider skip, snapshot redaction, async queue path (integration) | `automation_executions` |
| **A-6a** ✅ | n8n **authoring**: create/update/activate/delete + graph round-trip, capabilities on, signed triggers; enabling a workflow pushes it to the provider and stores the ref | `platform/automation/providers/n8n/`, `modules/automation/workflows/workflow-provider-sync.ts` | the A-0 conformance suite run against the n8n provider (n8n's REST contract stubbed); graph/ref/signing units; enable→push integration; opt-in live script | none |
| **A-6b** | Service tokens + callback action surface (n8n → ECMS) | `platform/automation/tokens/`, `modules/automation/actions/` | token scope = owner scope, no escalation; every callback audited as the owner | none |
| **A-7** | Executions: history, retry, cancel, progress callback, sweep (grows the `automation_executions` collection A-5 introduced) | `modules/automation/executions/` | retry idempotency, stuck-execution sweep | (collection exists) |
...
| **A-9a** | The **first three packages** as data, ahead of the installer: account-activation follow-up, password-reset follow-up, job-offer onboarding — plus the shared n8n error-handler workflow and a package validator/previewer | `automation/templates/`, `automation/n8n/`, `scripts/automation-templates.mjs` | every package parses against `AutomationTemplatePackageSchema` and its graph is reachable from `ECMS Trigger` (CI) | none (packages are data) |
...
| 2026-07-30 | 2.0 — A-6a delta | A-6 **split** on size: authoring (A-6a) is delivered here; service tokens + the callback action surface (A-6b) get their own review, since a per-execution credential that carries the owner's permissions is a security surface that deserves not to ride along with a REST client. A-6a: the n8n provider implements the full authoring half of A-0's interface, `visualBuilder`/`graphImportExport` flip **on**, and `cancellation`/`perNodeProgress` stay **off** until A-7 wires execution state back — a capability is a promise. Enabling a workflow now pushes it to the provider and stores the ref, so `providerRef: null`/`skipped` stops being the universal outcome; enable propagates errors, delete swallows them, and both are no-ops without a runtime. The provider ref encodes n8n's workflow id **and** the webhook path (opaque to ECMS, meaningful to the provider that minted it); an update rewrites the same path so a trigger URL never moves. Per-workflow webhook signatures are **derived** — `HMAC(deployment secret, webhook path)` — giving §2.2's per-workflow secret with no new column. Conformance runs against the n8n provider with n8n's REST contract stubbed, per A-0's rule that a provider needing a live runtime to pass conformance has made the runtime part of the interface; `npm run check:n8n` is the operator's opt-in end-to-end check against a real instance. |
| 2026-07-29 | 1.9 — A-5 n8n-client delta | On the approver's instruction the bridge now targets a real deployed n8n. Added a reusable `N8nClient` (the single hardened HTTP transport — `N8N_BASE_URL`/`N8N_API_KEY`/timeout/retry, base URL **never hardcoded**, never logs the key or body) and an `N8nAutomationProvider` behind A-0's interface: `dispatch()` POSTs to the workflow webhook and `health()` reports reachability, capabilities all-false (authoring rejects with `N8nNotImplementedError` until A-6). Registration is opt-in (`AUTOMATION_PROVIDER=n8n` + `N8N_BASE_URL`) and does **not** probe n8n at boot. Best-effort preserved: an unreachable n8n costs one logged, `skipped` trigger, never the business transaction. Scope held to the trigger path — no workflows built in n8n, no business automation logic (that is A-6/A-9). `.env.example`, the Railway deploy guide (§4.1) and the trigger architecture doc updated. Client/provider stay behind `providers/**` (the module-boundary lint rule); modules reach n8n only through `automationService`. |
| 2026-07-29 | 1.8 — A-5 async delta | The event handler no longer dispatches inline — it enqueues one `automation`-queue job and returns, and the worker resolves/filters/dispatches under BullMQ's retry policy. Closes the reliable-tier gap (the bus marks an event processed *before* the handler, so a synchronous failure was never retried); BullMQ retries with backoff and the execution unique index keeps retries idempotent, with `jobId = trigger_<eventId>` as a second dedup layer. Added a small platform seam, `ModuleManifest.jobHandlers`, registered generically at boot like `eventSubscriptions`, so a module owns a worker handler without the kernel importing it. |
| 2026-07-29 | 1.7 — A-5 delta | Trigger bridge subscribes ONE handler per cataloged event (generated from the catalogue), so automation is an ordinary consumer with no bus change. `automation_executions` is introduced HERE (the "→ execution rows" in A-5's goal) with A-7 growing its lifecycle — the plan's migration columns are reconciled accordingly. Idempotency is a unique `(eventId, workflowId)` index caught as a duplicate-key, not a check-then-write. The depth guard is enforced now though depth propagation from automation-emitted events lands at A-6; until A-6 pushes graphs, every dispatch records `skipped` (no provider ref). |
| 2026-07-29 | 1.6 — A-4.1 delta | Two approver requests, folded into A-4 before merge. (1) **Credential usage audit**: every `resolveForExecution()` open — success AND failure — writes an `automationCredentialUsed` audit row with credential/workflow/execution/provider/principal/branch/time/outcome, never the secret. (2) **Secret-store abstraction**: credential storage now depends on a `SecretStore` seam (`platform/secrets/`) holding an opaque `SecretRef`, not on `SealedValue` directly — so a KMS/vault backend is a new store + one boot line, with no automation-contract change. `platformCrypto` is the default store. |
| 2026-07-29 | 1.5 — A-4 delta | Injection into a running execution deferred to A-6, where a provider first exists to inject into; `resolveForExecution` is the seam and is reachable from no HTTP route. Rotation is a declared scheduled task (`automation.rotateCredentialKeys`, nightly) rather than a manual operation, which is what §7.3's "rotation without re-entering secrets" requires in practice. |
| 2026-07-29 | 1.4 — scope ruling | Approver ruled the HR payload divergence found at A-2.1 **out of scope for Automation**: existing domain behaviour, not an automation concern, and unifying it inside this milestone would mix domain refactoring with platform implementation. Recorded as TD-001 in the technical-debt register; the Automation layer stays compatible with the event contracts as they stand. Suspend-on-deactivate reconciliation likewise confirmed for A-7. |
| 2026-07-29 | 1.3 — A-3 delta | Trigger validation lands at A-3 (save time) rather than A-5 (dispatch time): telling an author their filter is wrong while they write it beats recording a skipped run three weeks later. Workflow `status` gains the documented `draft → active/disabled/suspended` transitions, with editing a live trigger dropping back to `draft`. |
