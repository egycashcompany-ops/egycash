...
| `takenInByEmployeeId` / `takenOutByEmployeeId` | refs, nullable | CUSTODY — who handed the car over (legacy driver/driver2), resolved from the authenticated user. Not the driver: see §4.2 |
| `notes` | string, nullable | |
...
**Driver is not custody.** `takenInByEmployeeId` / `takenOutByEmployeeId` answer "who handed the
car over", and are resolved from the authenticated user through the platform directory seam
(`getSelfDirectoryEmployee`) rather than typed in; a login with no employee behind it records
nothing rather than inventing somebody. The DRIVER shown against a visit is a different fact
entirely — the duty roster's crew for the vehicle on `inDate` (§2.7), read at query time.
...
| FR-4 | At most one open maintenance visit per vehicle; `outDate ≥ inDate`; check-out records custody and requires `exitOdometer ≥ odometerAtService` | legacy + tightened |
