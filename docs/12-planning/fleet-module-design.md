...
§8 event is live; all 22 `fleet.*` events are `stable`. What remains is the web tier and the
one-off legacy migration (FL-10), each starting only on the owner's go.
...
|---|---|---|
| **FW-1** | Skeleton: routes + guards + nav seed + i18n + api/query layer | ✅ |
| FW-2…FW-10 | The pages, in the order above | pending owner approval per slice |
