...
## 20-و. تنفيذ القائد التتابعي (OP-7) — **قدرة جديدة بالكامل**

لا نظير لهذا في Legacy إطلاقًا: لم يكن هناك سطح للقائد ليُنفِّذ منه شيئًا. الجديد هنا: **تنفيذ القائد، القفل التتابعي، تأكيد الاستلام/التسليم من الموبايل، وفرض التقدّم من الخادم**. أما ما تحته — الشحنة، الساقان، صف الطاقم `(يوم، مركبة)`، و`sequence` المحفوظ من OP-5 — فكله نقل/تطبيع من Legacy ولم تمسّه هذه الشريحة.

### آلة الحالة (تُعرَّف قبل التنفيذ)

```
pending ──start──▶ active ──confirmPickup──▶ pickedUp ──confirmDelivery──▶ delivered ──complete──▶ completed
```

لكل فعل **سَلَف شرعي واحد فقط**. هذه الحقيقة وحدها تجعل الممنوعات مستحيلة لا «مفحوصة»:

| ممنوع | لماذا هو مستحيل |
|---|---|
| إتمام شحنة لم تبدأ | `complete` لا يقبل إلا `delivered` |
| الرجوع للخلف | لا فعل يستهدف حالة أسبق |
| إتمام الشحنة مرتين | `completed` ليس سَلَفًا لأي فعل |
| تخطّي خطوة داخل المحطة | لا يوجد فعل من `pending` إلى `pickedUp` |

`cancelled` مُعلَنة في مفردات OP-1 لكنها **بلا انتقال هنا عمدًا**: لا شيء في النطاق المعتمد يلغي محطة، واختراع مسار إلغاء (من يملكه؟ ماذا يحدث للنقدية؟ هل يفتح التالية؟) قاعدة عمل لم يطلبها أحد.

### الفصل بين دورتَي الحياة

| | حالة الشحنة (business) | تنفيذ القائد (execution) |
|---|---|---|
| القيم | `draft/inVault/dispatched/completed` | `pending→active→pickedUp→delivered→completed` |
| المالك | الـback office | القائد |
| الصلاحية | `operationsShipment.complete` | `operationsExecution.own` |
| المكان | `operations_shipments` | `operations_shipment_assignments` (لكل ساق) |
| الأصل | Legacy | **جديد** |

**لا تكتب إحداهما الأخرى.** إتمام التنفيذ **لا** يُتِمّ الشحنة، لثلاثة أسباب: (١) لكان قائدٌ يحرّك سُلَّمًا لا يملك صلاحيته؛ (٢) الشحنة المحصنة لها ساقان بقائدين ويومين — انتهاء ساق لا يعني انتهاء الشحنة؛ (٣) لسُلَّم العمل دلالات إعادة فتح موروثة (Q30) يجب ألا يعبث بها التنفيذ.

الموضع الوحيد الذي تلتقيان فيه هو **قراءة** واحدة: `isStopSettled(execution, shipmentStatus)`. شحنة أتمّها الـback office لا تترك للقائد ما يفعله، فتتوقّف عن حجب مساره — ولولا ذلك لتعطّل المسار كله خلف خطوة لا يستطيع أحد أداءها. **نتيجة موثّقة:** هذا هو المسار الوحيد الذي قد تُرى فيه محطة لاحقة «منتهية» وأسبق منها لا — وهو فعل جهة أخرى مُخوَّلة، لا التفاف من القائد؛ تنفيذ القائد وحده لا يستطيع إنتاج هذه الحالة أبدًا.

### التتابع والتزامن

- **القفل على `start` وحده**: القفل يجيب «أي محطة يجوز أن **يبدأها**»، ومن بدأ يجب أن يُكمل. لو فُحص عند كل خطوة لأمكن لإعادة فتح من الـback office أن تُعلِّق قائدًا في منتصف محطة وبحوزته نقدية. و«البدء» هو اللحظة الوحيدة التي يمكن فيها خرق الترتيب أصلًا، لأن الخروج من `pending` لا يكون إلا به.
- **الترتيب مصدره واحد**: `sequence` من OP-5، يُقرأ عبر `orderedCaptainRoute` نفسه الذي تستخدمه قراءة اليوم — فلا تنفصل الشاشة عن القفل. لا آلية ترتيب ثانية.
- **الفتح مُشتقّ لا مخزَّن**: لا توجد راية «مفتوح» ليتسابق عليها طلبان. هذه خاصية بنيوية، لا حراسة إضافية.
- **الانتقال نفسه هو الـcompare-and-swap**: الحالة المتوقَّعة جزء من الـfilter، فمن متسابقَين يطابق مستندٌ واحد ويقع انتقال واحد؛ والخاسر يحصل على 409 لا على فوز ثانٍ. و`version` تُحترَم متى أرسلها العميل.
