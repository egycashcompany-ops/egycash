...
| **D4** | **One evaluator per review, and they are ASSIGNED.** Defaulted from the employee's department manager where there is one, and changeable while the review is `draft`. Not derived at read time: a manager who leaves in October must not silently change who reviewed somebody in June. |
